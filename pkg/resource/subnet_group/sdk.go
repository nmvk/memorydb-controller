	...
	for _, elem := range resp.SubnetGroups {
		...
		if elem.Subnets != nil {
			...
			for _, f3iter := range elem.Subnets {
				...
				if f3iter.AvailabilityZone != nil {
					f3elemf0 := &svcapitypes.AvailabilityZone{}
					if f3iter.AvailabilityZone.Name != nil {
						f3elemf0.Name = f3iter.AvailabilityZone.Name
					}
					f3elem.AvailabilityZone = f3elemf0
				}
				if f3iter.Identifier != nil {
					f3elem.Identifier = f3iter.Identifier
				}
				f3 = append(f3, f3elem)
			}
...
func (rm *resourceManager) requiredFieldsMissingFromReadManyInput(
	...
	return r.ko.Spec.SubnetGroupName == nil
...
func (rm *resourceManager) newListRequestPayload(
	r *resource,
) (*svcsdk.DescribeSubnetGroupsInput, error) {
...
func (rm *resourceManager) sdkCreate(
	...
	if resp.SubnetGroup.ARN != nil {
		arn := ackv1alpha1.AWSResourceName(*resp.SubnetGroup.ARN)
		ko.Status.ACKResourceMetadata.ARN = &arn
	}
	if resp.SubnetGroup.Description != nil {
		ko.Spec.Description = resp.SubnetGroup.Description
	} else {
		ko.Spec.Description = nil
	}
	if resp.SubnetGroup.Name != nil {
		ko.Status.Name = resp.SubnetGroup.Name
	} else {
		ko.Status.Name = nil
	}
	if resp.SubnetGroup.Subnets != nil {
		f3 := []*svcapitypes.Subnet{}
		for _, f3iter := range resp.SubnetGroup.Subnets {
			f3elem := &svcapitypes.Subnet{}
			if f3iter.AvailabilityZone != nil {
				f3elemf0 := &svcapitypes.AvailabilityZone{}
				if f3iter.AvailabilityZone.Name != nil {
	...
	if resp.SubnetGroup.VpcId != nil {
		ko.Status.VPCID = resp.SubnetGroup.VpcId
	} else {
		ko.Status.VPCID = nil
	}
...
func (rm *resourceManager) newCreateRequestPayload(
	ctx context.Context,
	r *resource,
) (*svcsdk.CreateSubnetGroupInput, error) {
...
func (rm *resourceManager) newDeleteRequestPayload(
	r *resource,
) (*svcsdk.DeleteSubnetGroupInput, error) {
